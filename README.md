...
- verification still mostly uses the metadata/token verifier plus default
  ...
  index types, call arguments with simple branch guards and attached Trust
  callee preconditions, `if` branch return facts, Option/Result match arms, and
  TrustModel field projections, field types, path guards, and struct returns
...
- `TRUST_SEMANTIC_VERIFY=0`: debug escape hatch that disables feeding default
  ...
  with element and index types, guarded call arguments with attached Trust
  callee preconditions, `if` branch return facts, Option/Result match arms, and
  TrustModel field projections in returns and path guards plus field types for
  arithmetic obligations, into verification.
