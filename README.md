...
- verification still mostly uses the metadata/token verifier; opt-in HIR/MIR
  extraction now feeds compiler-derived return expressions and arithmetic
  operations, slice index facts, and call arguments into verification, but it
  is not the primary VC generator yet
...
- `TRUST_SEMANTIC_VERIFY=1`: extracts rustc HIR/MIR and feeds supported MIR
  facts, including return expressions, arithmetic operations, slice indexes,
  and call arguments, into verification without writing dumps.
